// $Id: SimplexSolver.java,v 1.34 1999/12/10 21:29:39 gjb Exp $
//
...
public class SimplexSolver extends Tableau
{
  ...
  {
    EditInfo cei = (EditInfo) _editVarMap.get(v);
  ...
  for (int i = 0; i < _stayPlusErrorVars.size(); i++) {
    ...
    eVars.remove(_stayMinusErrorVars.elementAt(i));
      ...
      EditInfo cei = (EditInfo) _editVarMap.get(clv);
  ...
  protected final void deltaEditConstant(double delta,
                                         AbstractVariable plusErrorVar,
                                         AbstractVariable minusErrorVar)
  {
  ...
  _infeasibleRows.insert(minusErrorVar);
    ...
    for (Enumeration e = columnVars.elements(); e.hasMoreElements() ; ) {
      final AbstractVariable basicVar = (AbstractVariable) e.nextElement();
  ...
  protected final void pivot(AbstractVariable entryVar,
                             AbstractVariable exitVar)
       throws InternalError
  {
